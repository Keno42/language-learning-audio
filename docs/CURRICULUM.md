...
| `situation_fill` | construction | `{ slot = "item_id" }` fill(s) the `situation` names ("Ask if she speaks English." → `{ language = "ensku" }`): any exercise narrating the situation (situation-stage recall, `connect()`) uses them; other stages still generate freely. Required whenever the situation mentions one specific slot value; validated to name a real slot and an item carrying that slot's tag. Write a slot-independent situation instead when several fills fit |
